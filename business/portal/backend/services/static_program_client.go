...
// 配置项存放的是环境变量名（如 CAAM_STATIC_TOKEN），实际令牌值从该环境变量读取；
// 若环境变量未配置，记录告警并回退使用令牌名本身（保持既有行为，便于灰度期兼容）。
func staticProgramToken(params *StaticParams) string {
	...
	utils.Logger.Warnf("静态化程序访问令牌环境变量 %s 未配置，将回退使用令牌名本身", name)
	return name
}
...
func (s *StaticJobService) CallWithParams(ctx context.Context, params *StaticParams, method, targetPath string, query map[string]string) (*StaticProgramResult, error) {
	...
	if token := staticProgramToken(params); token != "" {
