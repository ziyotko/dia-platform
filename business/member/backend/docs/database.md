...
> **写入口径统一**：新增会员 / 审批通过 / 自助续证 / 管理员重发均经同一个写入函数（`createCertificateRow`，可传入事务），传事务时与外层业务同进同出；PDF 渲染一律在事务提交后进行。
> **自助续证前置条件**（`POST /certificates/renew`）：正式会员 + 当年已缴费 + 不存在 `active` 证书或其 `file_path` 为空，否则返回「当前已有生效证书，无需重复续证」。
...
### 默认机构关联等级 + 当年会费标准（缺失时自动补齐）

- `member_org_levels`：为**尚未配置任何关联等级**的机构，关联上全部会员等级（不会覆盖/删减已有配置）。原因：入会审批、管理员新增会员、会员「加入机构」都要求机构已配置等级，全新库若不带默认值，主流程无法走通。
- `member_fee_standards`：为每个会员等级补齐**当年**一条标准，金额为占位值 **2000.00 元**（仅补齐缺失的「等级 + 当年」组合，不覆盖已配置金额）。**上线前必须在后台「会费标准」改成实际金额**，否则会按 2000 元收费/展示。

> 以上两项每次启动都会检查补缺（幂等）；补齐时会在日志里打 `[WARN] 种子数据：…` 提示（含条数与默认金额），便于核对。
...
> **种子数据不包含**：`member_certificate_templates`（证书样式）。该项为可选配置，未配置时证书 PDF 使用默认版式（详见 §七）；建议按等级上传 PDF 模板。
