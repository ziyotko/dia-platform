...
### 升级说明（2026-09-23 第三批：种子默认配置 + 唯一索引自动化）

- **种子数据会为新库/未配置的库补齐两项业务前置配置**（幂等、只补缺失、**不覆盖**已有配置，补齐时日志有 `[WARN] 种子数据：…`）：
  1. **机构关联等级**：为「一条关联等级都没有」的机构关联全部会员等级；
  2. **当年会费标准**：为每个会员等级补一条标准，金额为占位值 **2000 元/等级**。
  ⚠️ **升级后请进后台核对这两项**：会费金额是占位值，务必改成实际标准（只影响之后新建的费用记录，已有记录需在「会费管理」中单独修改）。若某机构确实不应支持全部等级，请在「组织机构」中调整。
- **启动会自动补建 `member_fee_records(member_id, year)` 唯一索引 `uk_member_year`**（幂等，失败仅记日志不阻断启动）。**注意数据影响**：若历史库存在同一会员同年度的重复费用记录，启动时会**自动删除多余行（每组保留 `id` 最小的一条）**，并在日志里记录「存在 N 组重复…已清理 M 条」。
  - 介意自动删数据的：升级前先按下方「手工 SQL ①」自行预检/清理，或先 `mysqldump` 备份；只想手工建索引不想自动清理的，可先手工执行 ①，程序检测到索引已存在就不会再动数据。
- **遗留表 `member_password_resets`**（自助找回密码功能已下线）：`AutoMigrate` 不会删表，留着不影响运行；需要清理时执行下方「手工 SQL ②」。
