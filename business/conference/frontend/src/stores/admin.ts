...
export const useAdminStore = defineStore('admin', () => {
  const token = ref<string>(localStorage.getItem('conference-admin-token') || '')
  ...
  function setToken(val: string) {
    token.value = val
    localStorage.setItem('conference-admin-token', val)
  }
  ...
  function logout() {
    ...
    localStorage.removeItem('conference-admin-token')
    window.location.href = '/conference/admin/login'
  }
