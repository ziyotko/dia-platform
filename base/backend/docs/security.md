...
- 权限表 `path` 可配置完整路径 `/business_base/api/users/:id`，也可配置相对路径 `/users/:id`。
...
3. **白名单接口**：用户基础信息、菜单、权限、改密、仪表盘、我的应用、未读消息数等基础接口。
   - `GET /business_base/api/auth/info`
   - `GET /business_base/api/auth/menus`
   - `GET /business_base/api/auth/permissions`
   - `POST /business_base/api/auth/change-password`
   - `GET /business_base/api/dashboard/stats`
   - `GET /business_base/api/app-instances/my`
   - `GET /business_base/api/messages/unread-count`
   - 工作流「属于自己的数据」类接口（归属校验均在 service 层，详见 1.4.1）：
     - `GET /business_base/api/workflows/options`（发起流程需先选流程定义）
     - `POST|GET /business_base/api/workflow-instances`
     - `GET /business_base/api/workflow-instances/:id`
     - `POST /business_base/api/workflow-instances/:id/cancel`
     - `GET /business_base/api/workflow-tasks`
     - `GET /business_base/api/workflow-tasks/approver-options`（转办/加签的候选人选项，只返回本租户启用用户）
     - `POST /business_base/api/workflow-tasks/:id/approve` / `reject`
     - `POST /business_base/api/workflow-tasks/:id/transfer`（转办）
     - `POST /business_base/api/workflow-tasks/:id/add-approver`（加签）
...
- **接口字段**：`GET /business_base/api/auth/captcha` 返回 `{ captcha_id, captcha_img }`；
...
- **开关**：是否启用验证码由「系统设置 → 安全策略 → 登录验证码」控制（`settings.security.captchaEnabled`）。
  登录页先调 `GET /business_base/api/site-info` 获取 `captchaEnabled`，关闭时不渲染验证码输入框、不提交验证码字段，后端同样不校验。
...
`/business_base/api/app/:appCode/*path` 只挂 JWT，但 handler 内依次校验：
