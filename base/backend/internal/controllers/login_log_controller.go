...
func (ctl *LoginLogController) Delete(c *gin.Context) {
	var req struct {
...
func (ctl *LoginLogController) Clear(c *gin.Context) {
...
func (ctl *LoginLogController) Export(c *gin.Context) {
	status, _ := strconv.Atoi(c.DefaultQuery("status", "-1"))
	q := service.LoginLogListQuery{
		TenantID: c.GetUint64("tenantID"),
		Username: c.Query("username"),
	...
	filename := "login_logs_" + time.Now().Format("20060102_150405") + ".csv"
