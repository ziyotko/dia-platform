...
- 子应用前端解析 `base_token`，调用底座 `GET /base/api/v1/auth/info` 校验用户信息。
- 适合已有系统改动成本低的场景。
...
- 底座在请求头中注入当前用户信息：
  ...
  - `X-Base-Tenant-ID`
- 应用后端读取请求头完成鉴权与数据隔离。
- 代理路径：`/base/api/v1/app/{appCode}/{原应用API路径}`，实际转发地址为
  `{backendUrl}{API前缀}{原应用API路径}`（例如 `backendUrl=http://127.0.0.1:8081`、`API前缀=/api` 时，
  `/base/api/v1/app/demo/users` 会转发到 `http://127.0.0.1:8081/api/users`）。
